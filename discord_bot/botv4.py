...
# Liste de termes culinaires (ingrédients, ustensiles, lieux)
CULINARY_TERMS = [
...
async def help2(ctx):
    ...
    help_message = (
        ...
        f"  Exemple : `{PREFIX}relations tomate éplucher des tomates avec un couteau`\n"
        f"  Relations possibles : r.instr, r.manner, r.agent, r.object, r.location.\n"
        f"- `{PREFIX}help2` : Affiche ce message d'aide.\n\n"
        "**Notes :**\n"
        "- Le mot doit être un terme culinaire (ex. : tomate, couteau, cuisine).\n"
...
async def relations(ctx, mot, *, texte):
    ...
    for phrase in phrases:
        ...
        match_object = re.search(rf"{mot}\s+(épluche|cuit|coupe)\s+({'|'.join(CULINARY_TERMS)})", phrase)
...
async def on_command_error(ctx, error):
    ...
    if isinstance(error, commands.CommandNotFound):
        ...
        await ctx.send(
            f" Arguments manquants. Exemple : `{PREFIX}relations tomate éplucher des tomates avec un couteau`\n"
            f"Utilisez `{PREFIX}help` pour plus d'informations."
        )
    else:
        await ctx.send(
