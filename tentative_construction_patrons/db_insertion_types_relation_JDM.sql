INSERT INTO TYPES_RELATION (id, nom, description_) VALUES 
...
(12, 'r_flpot', '(relation interne) potentiel de relation'), 
...
(29, 'r_chunk_pred', '(interne) d''un prédicat quel chunk ?'), 
...
(41, 'r_has_conseq', 'B (que vous devez donner) est une CONSEQUENCE possible de A. A et B sont des verbes ou des noms.  Exemples : tomber -> se blesser ; faim -> voler ; allumer -> incendie ; négligence --> accident ; etc.'), 
(42, 'r_has_causatif', 'B (que vous devez donner) est une CAUSE possible de A. A et B sont des verbes ou des noms.  Exemples : se blesser -> tomber ; vol -> pauvreté ; incendie -> négligence ; mort --> maladie ; etc.'), 
...
(45, 'r_chunk_sujet', '(interne)'), 
(46, 'r_chunk_objet', '(interne)'), 
(47, 'r_chunk_loc', '(interne)'), 
(48, 'r_chunk_instr', '(interne)'), 
...
(81, 'r_pref_form', 'Indique une forme préférée'), 
(82, 'r_interact_with', 'Indique avec quoi antécédent peut interagir.'), 
(83, 'r_alias', 'Indique que les deux termes sont identiques, le 2e est un alias du premier. Utile pour synchroniser  des termes ou raffinement (éventuellement poly).'), 
(84, 'r_has_euphemisme', 'Moins intense, euphémisme. Exemple : être mort => être fatigué'), 
...
(106, 'r_has_color', 'A comme couleur(s)... chat r_color noir'), 
(107, 'r_has_cible', 'Cible de la maladie : myxomatose => lapin, rougeole => enfant, ...'), 
(108, 'r_has_symptomes', 'Symptomes de la maladie : myxomatose => yeux rouges, rougeole => boutons, ...'), 
(109, 'r_has_predecesseur-time', 'Qu''est ce qui peut PRECEDER temporellement (par exemple -  inverse de successeur) le terme suivant :'), 
(110, 'r_has_diagnostic', 'Diagnostic pour la maladie : diabète => prise de sang, rougeole => examen clinique, ...'), 
(111, 'r_has_predecesseur-space', 'Qu''est ce qui peut PRECEDER spatialement (par exemple -  inverse de successeur spatial) le terme suivant :'), 
(112, 'r_has_successeur-space', 'Qu''est ce qui peut SUIVRE spatialement (par exemple Locomotive à vapeur -> tender, wagon etc.) le terme suivant :'), 
(113, 'r_has_social_tie_with', 'Relation sociale/familliale entre les individus... (annotation pour la nature exacte : frère, mari, etc.) Julie Depardieu r_social_tie>fille Gérard Deparidieu ; Gérard Deparidieu r_social_tie>père Julie Depardieu'), 
...
(128, 'r_node2relnode-in', 'Relation entre un noeud (quelconque) et un noeud de relation (type = 10)  - connecte le noeud d''entrée - permet de rendre le graphe connexe même avec les annotations de relations'), 
...
(136, 'r_has_auteur-1', 'Quel sont les oeuvres de l''auteur suivant ?'), 
(137, 'r_processus>agent-1', ''), 
(138, 'r_processus>patient-1', ''), 
(139, 'r_processus>instr-1', ''), 
(140, 'r_node2relnode-out', 'Relation entre un noeud (quelconque) et un noeud de relation (type = 10) - connecte le noeud de sortie - permet de rendre le graphe connexe même avec les annotations de relations'), 
(141, 'r_carac_nominale', 'Pour un terme donné, souvent un objet, il est demandé d''en énumérer les CARACtéristiques nominales (nom) possibles/typiques. Par exemple, stylo => bille, feutre'), 
(142, 'r_has_topic', 'Thème lié à l''objet de départ, exemples : restaurant r_has_topic sushis, magazine r_has_topic bande dessinée'), 
(148, 'r_pourvoyeur', 'Le pourvoyeur est l''entité qui fournit l''object de l''action (le pourvoyeur est un complément d''objet indirect introduit par ''à'', ...). Par exemple dans - Le client demande une pizza à la serveuse - le pourvoyeur est ici la serveuse ...'), 
...
(150, 'r_has_beneficiaire', 'Le bénéficiaire est l''entité qui tire bénéfice/préjudice de l''action (un complément d''objet indirect introduit par ''à'', ''pour'', ...). Par exemple dans - La sorcière donne une pomme à Blanche Neige -, la bénéficiaire est Blanche Neige ... enfin, bref, vous avez compris l''idée.'), 
...
(153, 'r_has_prop', 'Pour le terme donné, il faut indiquer les noms de propriétés pertinents (par exemple pour ''voiture'', le ''prix'', la ''puissance'', la ''longueur'', le ''poids'', etc. On ne met que des noms et pas des adjectifs).'), 
...
(162, 'r_potential_confusion_with', 'Confusion potentielle avec un autre terme (par exemple, acre et âcre, détonner et détoner).'), 
...
(167, 'r_has_value', 'Une valeur associée à une propriété ou un objet'), 
(168, 'r_has_value>', 'Une valeur associée à une propriété ou un objet'), 
(169, 'r_has_value<', 'Une valeur associée à une propriété ou un objet'), 
(170, 'r_sing_form', 'La forme au singulier d''un terme. Exemple : chevaux r_sing_from cheval (pas de relation inverse)'), 
(171, 'r_lieu>origine', 'A pour origine B, saucisse de Toulouse r_lieu>origine Toulouse'), 
(172, 'r_depict', 'Que représente le mot ? (depiction) A représente B, par exemple une photo représente une personne, un visage, un paysage, etc.'), 
(173, 'r_has_prop-1', 'Pour propriété donnée, il faut indiquer les noms de choses pertinentes ayant cette propriété  (inverse de r_has_prop).'), 
(174, 'r_quantificateur-1', 'inverse de r_quantificateur'), 
(200, 'r_context', 'Relation de contexte entre un terme et un noeud contexte.'), 
(222, 'r_pos_seq', 'Séquence de parties du discours (Nom, Verbe, Adjectif, Adverbe, etc.). Exemple : belle maison => Adj: Nom:'), 
...
(1000, 'r_prev', '(interne)'), 
(1001, 'r_succ', '(interne)'), 
(1002, 'r_termgroup', '(interne)'), 
(2000, 'r_raff_sem-1', 'Inverse de r_raff_sem (automatique)'), 
(2001, 'r_learning_model', '(interne)'), 
(175, 'r_promote-1', 'Par quoi le terme suivant est favorisé ? Par exemple, une réaction chimique est favorisée par un catalyseur.');
